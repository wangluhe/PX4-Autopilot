 ...
 *
 * 工作流程：
 * 1. 逐字节读取，寻找帧头（0xFC 0x2C）
 * 2. 收集64字节后，进行帧校验
 * 3. 校验通过后，解析锁定状态和目标脱靶量
 * 4. 校验失败则滑动窗口继续搜索
 */
bool AttackVision::try_read_frame()
{
	...
	// 检查第二个字节是否为0x2C
	if (_buf_len == 2 && _buf[1] != FRAME_HEAD_1) {
		_buf_len = 0;
		return false;
	}
	if (_buf_len < FRAME_LEN) return false;  // 等待收满64字节
	...
	// 00=默认无效，01=锁定中，10=锁定预测，11=退出锁定
